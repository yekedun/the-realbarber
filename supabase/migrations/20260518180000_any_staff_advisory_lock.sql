-- H-1: Advisory lock missing on the "any staff" path in create_appointment_atomic.
--
-- When p_staff_id IS NULL, assign_any_staff selects an available staff member
-- without holding any lock. Two concurrent requests can both pass the
-- availability check, both attempt INSERT, and one hits the EXCLUDE constraint
-- → spurious P0001 / 409.
--
-- Fix: after assign_any_staff returns a staff ID, acquire
-- pg_advisory_xact_lock(hashtext(staff_id)) then recheck work-window and
-- conflict — the same lock-then-recheck pattern the specific-staff branch
-- already uses.
--
-- Only change vs 20260518170000: four lines added inside the v_staff_id IS NULL
-- branch, after the "no available staff" guard.
...
BEGIN
  ...
  IF v_staff_id IS NULL THEN
    ...
    END IF;
    -- Lock then recheck: assign_any_staff ran outside any lock; a concurrent
    -- transaction could have claimed this staff's slot between selection and now.
